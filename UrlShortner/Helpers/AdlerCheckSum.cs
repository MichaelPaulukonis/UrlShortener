...
/// found @ https://www.codeproject.com/articles/21083/adler-checksum-calculation
/// core routine equivalent to others found online
/// eg, http://nareshjaiswalgrd.blogspot.com/2016/08/convert-stringtext-to-adler32-checksum.html
/// thanks to @CodingHorror for his article https://blog.codinghorror.com/url-shortening-hashes-in-practice/
/// file-processing code jettisoned and new method add for my purposes
/// </summary>
namespace UrlShortener.Helpers
{
    ...
    public class AdlerChecksum
    {
        ...
        /// <summary>
        /// leave the majority of the code as we found it
        /// this wraps the class-level variable that requires action, then read
        /// into one step. And converts it to a (lowercase) hexadecimal string.
        /// </summary>
        /// <param name="s">massage string to checksum</param>
        /// <returns></returns>
        public string AdlerString(string s)
        {
            var bytes = Encoding.Default.GetBytes(s);
            MakeForBuff(bytes);
            return m_unChecksumValue.ToString("x");
        }
